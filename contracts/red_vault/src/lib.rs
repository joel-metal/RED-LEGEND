...
fn vault_red_ledgers(check_in_interval: u64) -> u32 {
    ...
    ledgers.clamp(VAULT_RED_LEDGERS, MAX_PERSISTENT_RED)
}
...
#[contract]
pub struct RedVaultContract;

#[contractimpl]
impl RedVaultContract {
    // --- admin/config ---
    ...
    pub fn create_vault(
        ...
        env.storage().persistent().extend_ttl(&key, VAULT_RED_THRESHOLD, VAULT_RED_LEDGERS);
        env.storage().instance().extend_ttl(INSTANCE_RED_THRESHOLD, INSTANCE_RED_LEDGERS);
        env.events().publish(
            (VAULT_CREATED_TOPIC,),
    ...
    /// timestamp, extending the vault's RED.
    ///
    /// # Arguments
    /// * `env` - The Soroban environment
    ...
    ///
    /// Anyone can call this function once the vault's RED has lapsed. The funds
    ...
    /// Checks the remaining RED and emits a warning event if near expiry.
    ///
    /// This function can be called by anyone to monitor vault expiry status.
    /// If the remaining RED is less than `EXPIRY_WARNING_THRESHOLD` (24 hours),
    ...
    ///
    /// # Returns
    /// The remaining RED in seconds (0 if expired)
    pub fn ping_expiry(env: Env, vault_id: u64) -> u64 {
        ...
        let red = Self::get_red_remaining(env.clone(), vault_id).unwrap_or(0);
        if red < EXPIRY_WARNING_THRESHOLD {
            env.events().publish((PING_EXPIRY_TOPIC, vault_id), red);
        }
        red
    }
    ...
    /// Returns the remaining time-to-live (RED) for a vault in seconds.
    ///
    /// The RED is calculated as the time remaining until the vault expires
    ...
    /// `None` if the vault does not exist or the RED has already lapsed.
    pub fn get_red_remaining(env: Env, vault_id: u64) -> Option<u64> {
        let vault = Self::try_load_vault(&env, vault_id)?;
    ...
    pub fn update_check_in_interval(
        ...
        vault.last_check_in = env.ledger().timestamp();
        Self::save_vault(&env, vault_id, &vault);
        // Explicitly re-extend the vault's persistent RED using the new (potentially
        // longer) interval so storage outlives the updated check-in deadline.
        let new_red = VAULT_RED_LEDGERS(new_interval);
    ...
    fn save_vault(env: &Env, vault_id: u64, vault: &Vault) {
        ...
        let red = VAULT_RED_LEDGERS(vault.check_in_interval);
        env.storage().persistent().set(&key, vault);
        env.storage().persistent().extend_ttl(&key, VAULT_RED_THRESHOLD, red);
    }
